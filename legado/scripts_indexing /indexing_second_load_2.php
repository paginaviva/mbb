...
/**
 * Google Indexing API - Second Batch REAL Load (Batch 2)
 * 
 * This script handles the ACTUAL submission of the SECOND batch of URLs (remaining after first 135)
 * to the Google Indexing API using URL_UPDATED notification type.
 * 
 * It reads sitemap.xml, sorts by date (oldest first), skips the first 135,
 * selects the rest, obtains an OAuth 2.0 token, and sends notifications one by one.
 * 
 * It logs detailed results to indexing_second_load_2.log and provides a visual summary.
 */
...
define('LOG_FILE', __DIR__ . '/indexing_second_load_2.log');
define('BATCH_SIZE', 135); // URLs to skip
...
echo "<pre>"; // Use pre for browser output formatting
echo "=== Google Indexing API - Second Batch REAL Load (Batch 2) ===\n";
...
// Select remaining (skip first 135)
$batchUrls = array_slice($allUrls, BATCH_SIZE);
$totalInBatch = count($batchUrls);

echo "\n--- BATCH 2 INFORMATION ---\n";
echo "Skipped FIRST " . BATCH_SIZE . " oldest URLs (Batch 1).\n";
echo "Selected REMAINING " . $totalInBatch . " URLs for processing (Batch 2).\n";
...
// 3. Initialize Log
$logHeader = "=== Second Batch REAL Load Execution - " . date('Y-m-d H:i:s') . " ===\n";
$logHeader .= "Target: Remaining URLs starting from position " . (BATCH_SIZE + 1) . "\n";
...
echo "\n";
echo "=============================================================\n";
echo "  RESUMEN DE EJECUCIÓN - SEGUNDO LOTE (BATCH 2 - REAL)\n";
echo "=============================================================\n\n";

// Datos Generales
echo "📋 DATOS GENERALES\n";
echo "   Script ejecutado: indexing_second_load_2.php\n";
echo "   Sitemap procesado: " . SITEMAP_PATH . "\n";
echo "   Total URLs en sitemap: " . count($allUrls) . "\n";
echo "   URLs procesadas en Batch 2: " . $totalInBatch . "\n\n";
