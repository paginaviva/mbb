...
<div class="home-grid-container-mobile">
    ...
    <!-- BLOQUE A: La jornada de ayer (solo último post) -->
    <?php if (!empty($home_data['block_a']['latest'])): ?>
        <section id="block-yesterday-mobile" class="home-block-mobile">

            <!-- Último Post -->
            <?php
            $latest = $home_data['block_a']['latest'];
            ?>
            <div class="block-title-mobile">La jornada de ayer</div>
            <ul class="post-list-small-mobile">
                <li>
                    <a href="<?php echo $latest['url']; ?>" target="_blank"
                        onclick="trackHomeClick('yesterday_latest', '<?php echo $latest['id']; ?>', '<?php echo addslashes($latest['title']); ?>', '<?php echo $latest['category']; ?>', 0)">
    ...
    <!-- BLOQUE: Otras Jornadas (movido aquí) -->
    <?php if (!empty($home_data['block_a']['others'])): ?>
        <section id="block-other-days-mobile" class="home-block-mobile">
            <div class="block-title-mobile">Otras Jornadas</div>
            <ul class="post-list-small-mobile">
