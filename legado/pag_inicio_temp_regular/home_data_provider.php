...
function get_home_data() {
    ...
    // Requerimiento v2: Total 10 artículos en "Lo más reciente". 
    // POSICIÓN #1: SIEMPRE el último "Resumen Diario".
    // POSICIONES #2-#10: Selección manual (9 items).

    $final_10_list = [];
    $all_posts_pool = $sorted_posts; 

    // 1. Obtener el Resumen Diario (Será #1 de la lista final)
    $latest_resumen = null;
    $resumen_id = null;
    ...
    foreach ($sorted_posts as $slug => $post) {
        if (isset($post['tags']) && is_array($post['tags'])) {
            ...
            if (in_array('resumen diario', $tags_norm)) {
    ...
    // Agregar #1 a la lista final
    if ($latest_resumen) {
        $final_10_list[] = $latest_resumen;
    }

    // 2. Obtener Selección Manual (Slots 2-10 -> 9 items)
    $manual_slots = [];
    ...
    if (file_exists($manual_selection_file)) {
        ...
        if (isset($selection_data['ids']) && is_array($selection_data['ids'])) {
            ...
            foreach ($selection_data['ids'] as $id) {
                // Evitar duplicar el resumen diario si el usuario lo seleccionó por error como #2-10
                if ($id === $resumen_id) continue;

                if (isset($posts_by_id[$id])) {
                    $manual_slots[] = $posts_by_id[$id];
                }
                if (count($manual_slots) >= 9) break;
            }
        }
    }

    // 3. Rellenar Manual Slots si faltan (hasta llegar a 9)
    if (count($manual_slots) < 9) {
        foreach ($sorted_posts as $slug => $post) {
            if (count($manual_slots) >= 9) break;

            $post = enrich_post($post, $slug);

            // Saltar resumen ya usado
            if ($post['id'] === $resumen_id) continue;
            
            // Chequear duplicados en manual
            $already_in = false;
            foreach ($manual_slots as $mp) {
                if ($mp['id'] === $post['id']) {
                    $already_in = true;
                    break;
                }
    ...
    // 4. Fusionar listas
    // $final_10_list ya tiene el #1. Le pegamos los 9 manuales.
    $final_10_list = array_merge($final_10_list, $manual_slots);
    
    // Si por alguna razón rarísima no había resumen diario, final_10_list tendrá 9 items. 
    // Podríamos rellenar el 10º, pero asumimos que siempre hay posts suficientes.
    ...
    // --- Lógica Bloque A (Resúmenes Diarios) ---
    // Se mantiene igual, pero usamos la variable $latest_resumen calculada arriba para optimizar
    $data['block_a']['latest'] = $latest_resumen;
    ...
    foreach ($sorted_posts as $slug => $post) {
        ...
        if (isset($post['tags']) && is_array($post['tags'])) {
            $tags_norm = array_map(function($t) { return mb_strtolower(trim($t), 'UTF-8'); }, $post['tags']);
            if (in_array('resumen diario', $tags_norm)) {
                $daily_summaries[] = $post;
            }
        }
    }
    $daily_summaries = array_values($daily_summaries);
    
    // Indices 1 a 5
    if (count($daily_summaries) > 1) {
        $data['block_a']['others'] = array_slice($daily_summaries, 1, 5);
    }
